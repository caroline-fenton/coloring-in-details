...
const stores = new Map([
  ["color-corner-v75", new Map([[live.url, "live"], [stale.url, "stale"]])],
...
const context = {
  ...
  caches: {
    ...
    match: async (req) => stores.get("color-corner-v75").get(req.url)
  },
...
await activation;
assert.ok(claimed);
assert.equal(stores.get("color-corner-v75").get(live.url), "live");
